        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000}} onClick={(e)=>{ if(e.target===e.currentTarget) setReqOpen(false) }}>
          <div style={{background:cardBg,border:`1px solid ${cardBorder}`,padding:24,borderRadius:12,width:400,maxWidth:'90%'}}>
            ...
            <div style={{marginBottom:14}}>
              ...
              <input value={reqTitle} onChange={e=>setReqTitle(e.target.value)} style={{width:'100%',background:bg,border:`1px solid ${cardBorder}`,color:text,padding:10,borderRadius:8,outline:'none'}} />
            </div>
            <div style={{marginBottom:18}}>
              <label style={{display:'block',color:textMuted,marginBottom:6,fontSize:14}}>Auteur</label>
              <input value={reqAuthor} onChange={e=>setReqAuthor(e.target.value)} style={{width:'100%',background:bg,border:`1px solid ${cardBorder}`,color:text,padding:10,borderRadius:8,outline:'none'}} />
            </div>
            <div style={{display:'flex',gap:10,justifyContent:'flex-end'}}>
              <button onClick={()=>setReqOpen(false)} style={{background:cardBg,color:text,border:`1px solid ${cardBorder}`,padding:'8px 14px',borderRadius:8,cursor:'pointer'}}>Annuler</button>
              <button onClick={()=>{ alert(`Demande envoyée !\nTitre: ${reqTitle}\nAuteur: ${reqAuthor}`); setReqOpen(false); setReqTitle(''); setReqAuthor(''); }} style={{background:accent,color:'#000',border:'none',padding:'8px 14px',borderRadius:8,cursor:'pointer',fontWeight:700}}>Envoyer</button>
            </div>
          </div>
        </div>
      )}
