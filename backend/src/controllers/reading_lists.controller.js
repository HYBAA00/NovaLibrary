...
exports.list = async (req, res, next) => {
  try {
    const [rows] = await pool.query('SELECT * FROM reading_lists WHERE user_id = ? ORDER BY created_at DESC', [req.user.id]);
    res.json(rows);
...
exports.remove = async (req, res, next) => {
  try {
    ...
    await pool.query('DELETE FROM reading_list_books WHERE list_id = ?', [id]);
...
exports.getBooks = async (req, res, next) => {
  try {
    ...
    const [rows] = await pool.query('SELECT b.*, rlb.added_at FROM reading_list_books rlb JOIN books b ON b.id = rlb.book_id WHERE rlb.list_id = ? ORDER BY rlb.added_at DESC', [listId]);
    res.json(rows);
