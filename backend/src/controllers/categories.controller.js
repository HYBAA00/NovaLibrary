...
exports.list = async (req, res, next) => {
  try { const [rows] = await pool.query('SELECT id, name FROM categories ORDER BY name ASC'); res.json(rows); } catch (e) { next(e); }
};
...
exports.update = async (req, res, next) => {
  try {
    ...
    if (!result.affectedRows) return res.status(404).json({ message: 'Categorie introuvable' });
